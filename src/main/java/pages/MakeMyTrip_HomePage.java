	...
	WebElement btnNextInCalendar;
	 ...
	 private void selectToCity(String city){
			...
			wait.until(ExpectedConditions.visibilityOfAllElements(DayPicker_Month));
			 ...
			 for(int i=0;i<=monthsize-1;i++) {
				 ...
				 if(actMonth.equalsIgnoreCase(month +" "+year)) {
					 WebElement date=driver.findElement(By.xpath("//*[@class='DayPicker-Month']//div[text()='"+actMonth+"']/../following-sibling::div[@class='DayPicker-Body']//div[@class='dateInnerCell']/p[text()='"+day+"']"));						
					 date.click();
				     break;
				 }
				 if(i==monthsize-1) {
		 ...
		 public void selectDepartureFromTime(String departureFromTime) throws InterruptedException {
			 Thread.sleep(500);
			 WebElement departureTime= driver.findElement(By.xpath("//div[@id='departure_group']//label/span[2][contains(text(),'"+departureFromTime+"')]/preceding-sibling::span"));
			 departureTime.click();
		 }
		 
		 public void selectArrivalAtTime(String arrivalAtTime) {
			 driver.findElement(By.xpath("//div[@id='arrival_group']//label/span[2][contains(text(),'"+arrivalAtTime+"')]")).click();
		 }
